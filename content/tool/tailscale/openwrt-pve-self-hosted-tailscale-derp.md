...
1. **外出时稳定回家**：继续使用 Tailscale 访问家中设备；家里已有动态公网 IP 时，OpenWRT 配合 Subnet Router 通常已经足够。
...
如果家里 OpenWRT 拿到了动态公网 IP，并且已经作为 Tailscale Subnet Router 宣告了家庭内网网段，那么外出访问家里设备通常已经够用。

典型路径：

```text
外部电脑 → Tailscale → 家中 OpenWRT → 家庭内网设备
```

这时家里 OpenWRT 是公网可达的一端，Tailscale 更容易和它建立连接。即使访问的是没有安装 Tailscale 的内网设备，流量也可以通过 OpenWRT 的 Subnet Router 进入家庭网段。

因此，如果需求只是“我在外面访问家里的 NAS、电脑、摄像头、打印机”，优先做：

```text
动态公网 + DDNS + OpenWRT Tailscale + Subnet Router
```
...
预期：`derp.example.com` 解析到当前家庭公网 IP。
...
| 协议 | 外部端口 | 内部目标 | 说明 |
|---|---:|---|---|
| TCP | 33445 | 192.168.50.3:33445 | DERP HTTPS |
| UDP | 3478 | 192.168.50.3:3478 | STUN |

如果使用其他高位端口，也可以，例如：

```text
TCP 55443 → 192.168.50.3:33445
UDP 53478 → 192.168.50.3:3478
```
...
检查 acme.sh 是否记录了 reloadcmd：

```bash
grep Le_ReloadCmd ~/.acme.sh/derp.example.com_ecc/derp.example.com.conf
```

预期：输出中包含 `docker restart derper`。
...
预期：`derper` 处于 running 状态，日志中没有证书、端口、socket 相关错误。
...
```text
外部设备 → Tailscale → OpenWRT Subnet Router → 家庭内网设备
```
