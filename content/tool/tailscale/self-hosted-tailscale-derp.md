...
在宿主机创建证书目录并启动容器。注意：将 STUN 和 TLS 端口全部移至 50000+ 高位以规避探测。
            ...
            "Nodes": [
                {
                    ...
                    "STUNPort":         53478,
                    "STUNOnly":         false,
                    "InsecureForTests": true, // 必须为 true 以跳过 CA 检查
...
由于跨城、跨运营商的长距离传输对大包极其敏感，**必须**手动调低虚拟网卡的 MTU。
...
- **Mac 端**:
    ...
    ```
    # 先 ifconfig 找一下 tailscale 对应的 utun 编号
