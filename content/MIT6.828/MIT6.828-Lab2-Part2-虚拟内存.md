...
tags: []
show_tags: true
created: Thursday, September 18th 2025, 4:51:11 pm
updated: Thursday, September 18th 2025, 4:51:20 pm
---
...
1. 在 [QEMU monitor commands](https://pdos.csail.mit.edu/6.828/2018/labguide.html#qemu) 页面熟悉 qemu 的操作命令
   2. xp/Nx paddr: 查看物理地址 paddr 处保存连续的 N个 words
   3. info registers: 查看寄存器
   4. info mem: 查看映射的虚拟内存地址和它们的访问权限
   5. info pg: 查看当前的页表结构：虚拟地址范围；区分是页目录条目(PDE)还是页表条目(PTE)；访问权限；物理地址范围
...
## [[MIT6.828-Lab2-Part2-Exercise4-实现pmap.c中管理页表的程序|Exercise4]]: 
...
1. 在 `kern/pmap.c` 中，实现如下函数的代码：
   ...
   5. page_remove()
   6. page_insert()
7. 之后 mem_init() 中的 check_page() 会测试这些页表管理程序
