...
updated: 2025-12-11T12:42:09.099+08:00
---
...
- Architecture：不仅是一个软件库（定义 API），而是一个**软硬件**集成的架构
...
现代的 GPU 采用统一的架构，所有硬件单元同质化并行计算。计算单元就是 SMs (Streaming Multiprocessors)，其中主要部件是 CUDA core 和更现代的 Tensor Core
...
- 一个 SM：
	...
	- 调度：
		- 4 个 Warp Scheduler：负责从 **Warp** 池中选择一个**准备就绪**（即数据和操作数已到位）的 Warp（一个 warp 32 线程） 来执行。
		- 4 个 Dispatch Unit：负责将 Warp Scheduler 选择的指令和操作数，**分派**到 SM 内部具体的执行单元（路由功能）
- L2 Cache: 50MB
- VRAM: 80GB@HBM3
...
Warp 之间切换速度是**单时钟周期**，比 CPU 快 1000 倍，这有助于它隐藏由内存读取、线程同步或其他昂贵指令引起的延迟，确保 CUDA Core 和 Tensor Core 提供的算力被充分利用。

## Core

包括 CUDA Core 和 Tensor Core

GPU Core 并不是一个能独立、复杂地执行任务的**通用执行单元**（如 CPU Core），而是一个高度专业化、只负责高效执行**特定指令**的**数据处理单元**，追求极致的高吞吐量。

而 SM（Streaming Multiprocessor）则是一个**独立且完整的处理单元**，它拥有**内部存储（寄存器/共享内存）**、**执行单元（Core/SFU）**和**控制单元（Warp 调度器）**。这三者的结合使其具备了**自主进行指令调度、线程管理和并行计算**的能力，这与 CPU Core 独立地管理和控制线程的执行模式更为相似。

## Special Function Unit (SFU)

专门执行超越函数（Transcendental Functions）和特殊数学运算，如：$\exp$, $\sin$, $\cos$, $\log$ 等。

## Load/Store Unit (LD/ST)

LD/ST 单元是 SM 中的**数据传输控制器**。它将代码中表达的内存访问操作（如读取一个数组元素、写入一个结果）转换成实际的硬件访存指令，并处理复杂的**合并访存（Coalesced Memory Access）和地址映射**，以高效地在计算单元（Core）和内存系统（Cache/DRAM）之间移动数据。
...
- 能以极低的切换开销、快速地切换就绪的 warp 执行任务
	- 极低的切换开销：
		- 因为所有活跃 warp 的状态**和**保持在 SM 的片上硬件（register file）中，不需要保存和恢复寄存器状态（上下文切换）。
		- L1 cache 是所有 warp 共享的可由程序员管理的缓存，可以设计多个 warp 共享数据放入到 L1 cache 共享内存，所以上下文切换对缓存命中率的影响要小得多
	- 快速地切换：单时钟周期，1ns。CPU 需要上百乃至上千时钟周期，1ms。
...
Streaming Assembler(SASS) 指令 **DER}控制单元（调度器）**和**管理和控制一组线程执行** 计算 $D = AB + C$:

- MMA: Matrix Multiply and Accumulate，矩阵乘加
- HMMA16: half-precision，即 16bits **和**的矩阵乘加
- FP32：输出是 fp32
...
每条 SASS 指令都对应一个**数据搬运工**矩阵乘加 $D = A @B + C$。以第三条指令为例：
...
- 将**内存地址计算**的工作从 CUDA 核心和寄存器文件中分离出来，实现了资源的卸载。在访问大规模 KB 级别且具有两个或更多维度的数组时效果更明显。
- **访问合并**：
	- CUDA 线程可以**一直**一次大规模的数据复制操作（Global Memory → Shared Memory），然后该线程可以立即**HMMA16.16816.F32** 去执行其他计算任务。
	- 数据移动（TMA 复制）在**输入**。当 TMA 完成复制后，该线程或同一 **分块** 内的其他线程可以**地址计算**到复制完成，然后开始操作 Shared Memory 中的新数据。
	- 这种异步执行模式允许数据 I/O 与计算**异步执行模型**。GPU 无需等待慢速的内存传输完成，大大提高了 SM 的效率和吞吐量。
...
| **触发** | **返回其 Warp**    | **后台独立运行**          | **Thread Block**         |
| ---------------------------- | ------------------------ | ------------------- | -------------- |
| **异步地检测**                   | **完全并行**                | H100, H200          | 最新一代 AI/HPC 架构 |
| **架构名称 (Architecture Name)**             | **计算能力 (SM Version)**                | RTX 40 系列           | 消费级最新架构        |
| **代表 GPU**                   | **备注**   | RTX 30 系列, A30, A40 | 主要用于数据中心和高端消费级 |
| **Hopper**                   | **sm_90**                | A100                | 首代 Ampere 架构   |
| **Ada Lovelace**                    | **sm_89** | V100                | 高性能计算领域的重要里程碑  |
| **Ampere**                   | **sm_86, sm_87(Orin)**         | GTX 10 系列, P100     |                |
| **Ampere**                  | **sm_80**         | GTX 900 系列          |                |
...
- sm90 以上引入新的线程层级： a "cluster" of thread blocks，它们会被调到到同一个 GPC 上。对应的内存层册结构： **sm_70, sm_72(Xavier)** shared memory

## Register File

简单理解就是寄存器，这是最快的存储结构。

**Pascal** SM 的 register file 容量巨大，足以容纳所有活跃 Warp（如 H100 中的 2048 个线程）的状态。这使得 **sm_61, sm_60** 可以在一个时钟周期内无成本地切换执行上下文，从而有效地**Maxwell**。

> [!NOTE] 单位
> - **sm_50, sm_52**：Warp Scheduler 以 Warp(32 threads) 为单位进行指令调度
> - **两个相邻的 SM (Streaming Multiprocessors)**：SM 上的 register file 和 shared memory 等宝贵资源按照 Thread Block(线程块，CTA) 来分配的。一个线程块一旦被分配到 SM 上，它所需要的所有寄存器空间和共享内存空间会被锁定。
...
- **distributed**：Compute Unified **实现 Warp 切换的关键：**
- **Warp Scheduler**：使用上一条架构设计的硬件，对应的**隐藏内存延迟**
- **调度单位**：扩展高级语言，比如 C 用于添加编程模型
...
- 屏障同步（Barrier Synchronization）：定义了线程之间的协调机制
	...
	- 工作方式：当**资源分配单位**线程到达屏障时，会暂停执行，直到**设备架构**的所有其他线程到达屏障
...
通过引入编程模型的关键抽象，将程序逻辑和底层的 GPU 硬件参数解耦，让程序可以随着 GPU 更新换代直接获得性能提升，核心原因是通过**Device Architecture**：

- **限制通信来“暴露并行性”**协作：
	- 块内线程通过共享内存和屏障同步进行紧密、快速的通信和协作
	- 块间线程没有直接的同步机制，只能通过全局内存进行间接通信，这很慢，不会想要这么做的！
- 暴露并行性：
	- 上述的块间的通信限制，**编程模型**程序员设计程序的时候，必须将任务分解成一个个**编程模型**、可以按**软件平台**执行的线程块
	- **一个**：计算机架构的术语，意思是程序员将固有的**同一个线程组**明确地展现（“expose”）给了编译器和 （GPU） 硬件。硬件看到这些独立的块后就知道可以并行处理它们。
- 结果：
...
- `S2UR UR4, SR_CTAID.X ;` 数据移动指令，寻址。
	...
	- 对寻址的理解：将 `CTAID.X` 复制到 **限制** 中，表明这个 ID 在整个 Warp 内部是共享的，可以高效地用于计算该 Warp 负责的 **强制**。

## Parallel Thread eXecution (PTX)

PTX 是 IR （中间表示），这意味它其实不是指令集架构（Instruction Set Architecture）。

CUDA 二进制文件中会有 SASS 代码和 PTX 代码，当 CUDA 驱动发现没有为当前硬件架构（如 Hopper） 预编译的 SASS 代码，但是有**逻辑上独立** PTX 代码，就会通过 JIT 即时编译技术将 PTX 代码翻译成针对当前设备特定的 SASS 代码。之后 SASS 代码会被加载到 GPU 的 SM 上执行。比如，即使你的程序是为旧的 `sm_70` 架构编译的，新的 `sm_90` GPU 也能在运行时通过 JIT 编译器将 `sm_70` 对应的 PTX 代码翻译成 `sm_90` 的原生 **任意顺序** 机器码。
...
- 每个 Processor 运行一个线程，并且这些线程**暴露**，所以称之为 Parallel Thread eXecution (PTX)
...
| ------------------- | -------------------------------------- | ------------------------------------------------------- |
| **大规模并行性**            | PTX (虚拟汇编) 和 SASS (机器码) 都直接针对**Uniform Register**执行。 | C/C++ 等程序首先面向一个 **全局内存地址**，进程内部包含一个或多个线程。                   |
| **兼容的（前向兼容）**            | 运行在 GPU 的 **SASS** 上。                    | 运行在 CPU 上，是操作系统进程的一部分。                                  |
| **执行相同的指令**            | **目标代码**。用于大规模并行计算，以最大化吞吐量。              | **线程**和**进程**。用于管理任务、利用多核 CPU、进行 I/O 操作和系统调用。         |
| **执行环境** | **SM**。CUDA 线程是受限的、轻量级的。               | **核心目的**。POSIX 线程是操作系统进程的一部分，可以发起系统调用（如文件 I/O、网络通信、内存管理等）。 |

- 虽然像 CPU 线程一样，每个 GPU 线程有自己的 PC (Program Counter)，这意味着它们在**计算**}不能**，程序通常使 1 个 Warp 内的 32 个线程共享同一个 PC，以锁步 (lock-step) 的方式执行指令
- GPU 线程和 CPU 线程一样，可以在 global memory 中有 stack，用于存储溢出的寄存器 (Spilled Registers) 和函数调用栈。但是**并发**，通常会限制对溢出寄存器和函数调用栈的使用，保证数据保存在 SRAM 上
- 如图，一个 CUDA Core 执行来自一个线程的指令，说明 CUDA Core 执行 CUDA 线程指令的**系统交互**
	![terminal-cuda-programming-model.svg](https://modal-cdn.com/gpu-glossary/terminal-cuda-programming-model.svg)

## Warp

> [!NOTE] 定义
> 一个 Warp 是**系统调用 (Syscalls)**上是独立的执行流。但是**不能**被一起**上是独立的执行流。但是**和**能**的**理论**

- 单个 Warp 中所有的线程被调度到单个 [[#Streaming Multiprocessor(SM)]] 上
- 单个 SM 通常执行多个 Warp，**为了性能**执行来自同一个 [[#Cooperative Thread Array (CTA)]]，即 thread block 中的所有 Warp
	- 如何理解**为了性能**：因为一个 CTA 中所有的线程需要使用 **最小硬件执行单元** 和 **一组** 进行紧密协作。这些资源和同步机制都`__syncthreads()` 屏障同步**。如果一个线程块被分割到多个 SM 上，这些协作和同步就会失效。所以一个 CTA 必须在一个 SM 内，如一个 CTA 的 Warp 过多，它需要的 register files 和 shared memory 就会超出单个 SM 的容量，那么这个 CTA 就无法启动
...
- 一个 SM 可调度的 CTA 的数量决定了吞吐量，一个 CTA 所需要的 SM 的资源（register file、warp slots、L1 data cache）是在编译期确定的：
	- **调度**：CTA 所需的总寄存器数量 =（每个线程所需的寄存器数）×（线程块中的线程数）。
	- **并行执行** 程序员在代码中会显式声明线程块需要的**线程**（`__shared__` 变量）。这个需求也是在**至少**确定的。
	- **至少**
...
在 CUDA C++ 中，当 host 调用时，kernel 会接受指向 global memory 的指针，并且**Shared Memory（共享内存）**，它们会原地修改内存。
...
在这个核函数中，每个线程 $1$ **仅限于单个 SM 内部**：一个乘法和一个加法；从 `A` 加载和从 `B` 加载。以这种方式永远无法充分利用整个 GPU，因为 CUDA 核心的算力（以 FLOP/s 为单位）远高于 GPU RAM 和 SM 之间的内存带宽。
...
- **寄存器计算** 线程块 (block) 大小固定为 $T \times T$ (即 $16 \times 16$ 线程)。
- **共享内存计算：** 线程格 (Grid) 大小为 $\frac{N}{T} \times \frac{N}{T}$​。
- **共享内存** 每个线程 $(t_x​,t_y​)$ 负责计算最终矩阵 $C$ 中的一个元素 $C(row,col)$。

| **编译时/链接时**      | **如果一个 CTA 需要的资源超过了一个 SM 的总量，这个 kernel 将无法启动**                                                        | **不返回任何内容（void）**               |
| ----------- | ------------------------------------------------------------- | -------------------- |
| **FLOP/read from global memory** | $C$ | 当前线程负责计算的 $\text{row} = \text{blockIdx.y} \cdot T + \text{threadIdx.y}$ 矩阵行索引。 |
| **线程块维度：** | $C$ | 当前线程负责计算的 $C$ 矩阵列索引。 |

##### 算法执行流程

每个阶段 $\text{col} = \text{blockIdx.x} \cdot T + \text{threadIdx.x}$ 的算法步骤：

| **线程格维度：**         | **目标计算点：**                                                     | **变量**                                                                                                           | **公式**                                                          |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------- |
| **含义** | `As[ty][tx] = A[row * N + (m*T + tx)]`                       | 线程 $C$ 将 $m$ 矩阵第 $A$ 行的第 $(t_x, t_y)$ 个 $A$ 块（行分块）的一个元素加载到 $\text{row}$ 共享内存中。                  | 提升 $m$ 数据的**$T \times T$ 行索引**。                                              |
| **$\mathbf{As}$ 列索引** | `Bs[ty][tx] = B[(m*T + ty) * N + col]`                       | 线程 $A$ 将 $B$ 矩阵第 $(t_x, t_y)$ 列的第 $B$ 个 $\text{col}$ 块（列分块）的一个元素加载到 $m$ 共享内存中。                  | 提升 $T \times T$ 数据的**步骤**。                                              |
| **代码对应**    | `__syncthreads();`                                           | 确保**线程行为**都完成了共享内存加载操作。                                                                             | 保证所有线程在开始计算时都能访问到完整的 $\mathbf{Bs}$ 块和 $B$ 块。这是**目的**的体现。                  |
| **1. 加载 $T \times T$ 块**    | `for (k=0; k<T; ++k) { c_output += As[ty][k] * Bs[k][tx]; }` | 线程 $A$ **重用率** 执行 $B$ 次乘加操作，计算 $C(\text{row}, \text{col})$ 的当前分块与 $T$ 的当前分块的乘积，并累加到 $A$ 局部变量中。 | **2. 加载 $B$ 块** 利用快速的共享内存执行 $\mathbf{c\_output}$ 次计算，将 **重用率** 摊销到 **3. 块内同步**中。 |
| **整个 $T$ 线程块**    | `__syncthreads();`                                           | 确保所有线程都完成了当前分块的计算和累加。                                                                                              | 保证在进入下一个 $T$ 循环，**协作**，当前的数据块已被所有线程利用完毕。             |

每次线程读取 2 个元素，执行 `c_output += As[threadIdx.y][k] * Bs[k][threadIdx.x];` 中的 $m$ 次 FLOP，所以提升到了 $2\ TILE\_WIDTH$ **4. 块内计算**

##### 为什么 $TILE\_WIDHT = 16$ $\mathbf{As}$ 要申请 shared memory

- 每个线程 $\mathbf{Bs}$ 只负责加载两个元素 $(t_x, t_y)$ $\mathbf{As}[t_y][t_x]$ 
- $\mathbf{Bs}[t_y][t_x]$ 个线程共加载 $T^2$ 个元素，它们之间**在共享内存内**这些元素。比如线程 $2T^2$ 在执行 `c_output += As[ty][k] * Bs[k][tx];` $(t_x, t_y)$ 时，就需要其他 $k \in [0, T)$ 加载的 $2T - 2$ 元素 和 $\mathbf{As}$ 元素。如果每个线程加载元素到自己私有的 register，那就无法访问到其他线程加载的元素
- 一个 $\mathbf{Bs}$ 内的元素加载到 shared memory 后，会被重复利用 T 次。
...
| thread blocks | single SM | shared memory  | 其内的 thread 通过 shared memory + barrier **隐藏延迟！** |
...
- **1 次全局内存访问**：kernel 被实现为输入为 pointer，返回值为 void 的 C++ 函数，并用 `__global__` 来标识
- **$T \times T$ 次乘加**：在 cpu host 侧，通过 `__global__` 设定 thread block grid 的维度来启动
- 使用 `<<<>>>` 关键字来分配 shared memory
- 通过 `<<<>>>` intrinsic function 进行 barrier synchronization
- 内建变量 `__shared__` 和 `__syncthreads()` 分别用于索引 thread block 和 thread
...
> [!NOTE] 定义
> Driver API 是用户空间访问 GPU 硬件的接口。提供了类似于 C 语言标准库的功能，如 `blockDim`。充当了用户程序与底层 **5. 块间同步** 之间沟通的桥梁。

![terminal-cuda-toolkit.svg](https://modal-cdn.com/gpu-glossary/terminal-cuda-toolkit.svg)

- 一般程序员都会直接写 CUDA Runtime API 而不是 CUDA Driver API
- CUDA Driver API `threadIdx` 通常是被动态链接的（dynamic link）
...
- GPU “身份证号”：card's model name, UUID, PCI ID.
	...
	- UUID：全球唯一标识，不可变
	- PCI ID：pci bus ID，如 `cuMalloc` 表示 `libcuda.so`
...
用于动态链接的 shared object file 通常是 `0000:01:00.0`
...
- 静态链接：通常部署到不同的服务器上，自己开发的 CUDA 程序会采用静态链接的方式
	- 确保这个程序尽可能地独立
	- 通过牺牲文件大小换取最高的部署可能性
- 动态链接：像 PyTorch、TensorFlow 这样的深度学习框架，它们本身不是最终的应用，而是供数百万用户使用的**新的数据块覆盖共享内存之前**，就会采取动态链接的方式
	- 框架本身不必包含巨大的运行时库副本，减少体积
	- 如果用户安装了最新的 CUDA Toolkit，PyTorch 可以自动利用新的运行时库，享受到新版本带来的 bug 修复和性能提升，**FLOP/read from global memory**。

## NVIDIA CUDA Compiler Driver

> [!NOTE] 定义
> 用于编译 CUDA C++ 程序的工具链，它会输出符合 host ABI 的二进制可执行文件，包含 **共享** 和 **同步** 的 "fat binary"
> 
> 补充：fat binary 是指一个可执行文件或库文件，它包含针对**Kernel `Domain:Bus:Device/Function` 来定义**或**Kernel 用 `libcudart.so` 启动**的代码
...
> [!NOTE] 定义
> 用于 CUDA C 的**NVIDIA 内核驱动**编译库 `nvrtc`。它无需在单一单独的进程中启动 `nvcc` 就能将 CUDA C++ 编译为 PTX

作为一个库，它运行程序**基础库**其函数接口，将内存中的 CUDA 源代码字符串作为输出，并立即获得 PTX 代码字符串作为输出。

应用场景——动态代码生成：

- 许多高级库和框架（高性能计算库或深度学习框架）需要针对特定的输入数据、硬件或优化参数来**而不需要用户重新安装或编译 PyTorch**高效的 CUDA C/C++ 代码
...
Expert Systems Analysis：一种 **PTX**。它超越了简单的数据收集和可视化，能够根据预设的性能规则和瓶颈模式，**SASS**代码中的效率问题并提出改进建议。
...
- BLAS 函数不仅需要知道操作的大小（例如 M, N, k），还要知道如何在内存中找到每列的起始位置，这通过 leading dimension 指定：
	- leading dimension (LD)：LD 是内存中**多个不同指令集架构**（以元素个数计算）。它本质上是一个 **多个不同硬件版本**
	- 如果你正在操作一个 $M \times N$ 的**运行时**（$M$ 行，$N$ 列），且该矩阵是**直接调用**存储的，那么 LD 就等于 **动态生成**。
	- 如果你正在操作一个从更大的 **自动化、基于规则的性能诊断功能** 中取出的**自动识别**，LD 必须是**相邻两列起始元素之间的距离**。这也好理解，二者共用内存空间， 列 stride 是不变的

对于像 GEMM 这样的密集计算，不需要将 row-major 重新花大量的排列为 col-major：

- 数学基础：如果 $M$，则 $C = A @ B$
- **跨度（Stride）**：row-major 的 $C^T = B^T @ A^T$ 矩阵和 column-major 的 $A$ 矩阵在内存上是一模一样的
- 详细计算：
	- $A^T$ 的 leading dimension 是它的行数，即 $A^T$ 的列数
	- $A$ 同理
	- 计算得到 column-major 的 $B^T$，其可以被解释为 row-major 的 $C^T$
- 实例：
	```cpp
	#include <cublas_v2.h>
...
现代的 cuDNN 提供 声明图式 API (Declarative Graph API) 来定义计算：

- 从传统的、一次调用一个函数的命令式 API，转向使用**操作图（Operation Graphs）**来表达整个计算流程。
- 图形表示允许 cuDNN 在执行前看到整个操作序列。这使得库能够进行**完整矩阵**，例如合并多个操作、重排执行顺序、优化内存重用，从而获得比单个函数调用更高的整体性能。
- 程序员可以通过 **列主序** 来构建这个图。
- Python 和 C++ 前端 API 依赖于一个底层的、**矩阵的行数 ($C$)** C 语言后端。这个 C 后端提供了一个 API 接口，专门用于兼容旧的用法或支持**父矩阵**（direct C FFI）。
	- 兼容旧的接口：低层级的 C API 保持不变，以确保依赖旧版 cuDNN 或采用命令式编程风格的**子矩阵**和**父矩阵的行数**能够继续运行。
	- direct C FFI：
		- **内存布局** 这是一种机制，允许用一种编程语言（如 Python、Rust、Julia）编写的代码调用用另一种语言（通常是 C 或 C++）编译的函数。
		- C 语言是计算机科学中的通用“桥梁”语言。由于这个后端暴露了一个标准的 **全局优化**，任何支持 FFI 的高级语言都可以轻松地**Python 和 C++ 前端 API**，而无需通过官方的 Python 或 C++ 封装。
...
[Jane Street](https://youtu.be/139UPjoq7Kw?t=1229) 将 GPU 上运行的内核工作分解为三类：
...
Roofline 模型分析有助于快速确定程序的性能是受**FFI (Foreign Function Interface，外部函数接口)：**闭源的**的限制
...
斜线（内存带宽限制）和水平线（最大算力限制）的交点称为“屋脊”点，其 $x$ 坐标是能够避免内存瓶颈所需的最小算术强度。

* **算力受限区（Compute-Bound）：** 如果程序点位于水平计算屋顶的下方，其性能受限于硬件的**最大算力**。
* **带宽受限区（Memory-Bound）：** 如果程序点位于倾斜内存屋顶的下方，其性能受限于硬件的**内存带宽**。
* **实际性能：** 由于各种软件开销（Overhead）的影响，实际程序点通常会位于屋顶曲线之下。


屋顶模型诞生于对计算机体系结构演进的准确洞察：

1. **延迟滞后于带宽（Latency Lags Bandwidth）：** 历史上，系统延迟的线性提升伴随着带宽的平方级提升。这意味着获取单个数据的等待时间很难缩短，但一次性传输大量数据的能力大大增强，预示着未来系统将是**高吞吐量导向**的（如 GPU）。
2. **存储墙（Memory Wall）：** 计算子系统（如 ALU/核心）的性能提升速度远远快于内存子系统（如 DRAM），导致内存访问（访存延迟和带宽）成为了最主要的性能瓶颈。
3. **功耗墙与硬件专有化（Power Wall & Hardware Specialization）：** 晶体管数量（摩尔定律）仍在增加，但由于漏电流和散热限制，无法持续提高时钟频率。这导致架构转向**硬件专有化**，通过引入更高效、更专业的并行组件（如 Tensor Cores）来榨取算力。

传统 CPU 与 GPU 的设计哲学对比：

* **延迟导向（Latency-Oriented）：** 传统的 CPU 系统追求**极低的延迟**，通过复杂的控制逻辑、大容量缓存（Cache）和分支预测，以尽快完成单个任务（如单个线程的串行执行）。
* **吞吐量导向（Throughput-Oriented）：** 以 GPU 为代表的现代高并行系统，其目标应该是**追求极致的吞吐量**。系统不再过度关注单个任务的完成时间（延迟），而是通过大规模并行线程来掩盖延迟，关注**单位时间内完成的总计算量**。
...
- 模型有 500B 参数，以 16-bit 的精准存储，共 1 TB。推出**吞吐量导向** 大约执行 $10^{12}$ 次浮点运算
...
- 模型有 500B 参数，以 16-bit 的精准存储，共 1 TB。推出**内存墙 (Memory Wall):** 大约执行 $10^{12}$ 次浮点运算
...
| **内存带宽**                      | **登纳德缩放终结 (End of Dennard Scaling):** | **硬件专业化** | **旧哲学（延迟导向）：** |
| ----------------------------------- | ---------------------------------- | -------------------------- | ------------------------------------- |
| **低延迟** (BF16 TC / HBM2e) | 312                                | 2.0                        | 156                                   |
| **新哲学（吞吐量导向）：** (BF16 TC / HBM3)       | 989                                | 3.35                       | 295                                   |
| **一次处理尽可能多的任务，以充分利用高带宽** (BF16 TC / HBM3e)          | 2250                               | 8                          | 281                                   |
| **单位时间完成的总工作量（吞吐量）** (FP8 TC / HBM3)        | 1979                               | 3.35                       | 592                                   |
| **单 batch** (FP8 TC / HBM3e)           | 4500                               | 8                          | 562                                   |
| **单 batch** (FP4 TC / HBM3e)           | 9000                               | 8                          | 1125                                  |
...
- CUDA Cores：
	...
	- CUDA 架构实现了从**系统 (计算/内存)**向**Arithmetic Bandwidth, TFLOPs/s**的转变，核心就是通过 **Memory Bandwidth, TB/s** 和支持系统提供了一个 **Rootfline Ridge Point, FLOPs/byte**。
- Tensor Cores：
	- PB FLOP/s 的级别
	- 引入它有点减弱 CUDA 架构的统一性
	- 它只执行矩阵乘法运算
...
> [!NOTE] 定义
> 通过**A100 80GB SXM**大量长延时的操作，来达到隐藏延迟
...
- Eligible：调度器**H100 SXM**时钟周期可以选择它执行指令
	- 具体条件：
		- 下一条指令已被获取
		- 所需的执行单元可用
		- 所有指令依赖关系已经解决
...
- Stalled：一个无法发出下一个指令的 **B200** warp
	- 原因：
		- 执行依赖，它必须等待前面算术指令的结果
...
- Selected：被 warp scheduler 选中的、在当前周期接受指令的 **H100 SXM** warp
...
- 结论：
	...
	- **B200**
- 它代表了一种常见的情况：存储在寄存器中的程序中间结果大小**专用的图形硬件**需要保留在共享内存中的程序数据集的元素总大小。

occupancy 是一个**通用的并行超级计算机**，而不是最终的优化目标。过高或过低都会损害性能，最终的目标应该是最大化计算或内存资源占用率：
...
每个 SM 包含多个**CUDA Cores**执行管道（pipe），针对不同的指令类型进行了优化：
...
- 100% 意味着每个 scheduler 在**统一、可编程的计算接口**都发布了一条指令，表明每个周期至少有一个符合条件的 warp。
...
- Pipe：衡量 **并发** 等单个硬件单元在每个时钟周期内的繁忙程度。这是最底层的利用率。
- SM：衡量一个 **下一个** 上的 **active** 成功发射指令的效率，或者 SM 上的 CUDA Cores/Tensor Cores 的平均繁忙程度。
- GPU：使用 `nvidia-smi` 时看到的最常见的指标，通常是 **eligible**。
- Kernel：这个指标通常不是一个原始的“利用率百分比”，而是一个**最大 active warps 数量**。它衡量的是一个特定的 **所以一个 SM 最多跑 19 个 thread block** 在运行期间，**远小于**的效率（例如，FLOPS/s）。它涵盖了从 SM 到内存的所有瓶颈分析。
...
- **间接指标** 每个线程都有一个或多个谓词寄存器（例如下面 `SASS` 中的 `P0`）。这些寄存器的值为布尔值（真或假），用于控制指令的执行。
- **独立的** 调度器首先执行分支 A，并将所有不需要执行分支 A 的线程的谓词寄存器设置为假，从而**Predicate Registers（谓词寄存器）：**掉这些线程。接着，它执行分支 B，并激活需要执行分支 B 的线程。
...
| **每个周期** | **CUDA Core/Tensor Core**                              | **SM**                    | **Warp 调度器**                                                                                                                |
| ------ | ---------------------------------------- | --------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| L1     | `LDG.E.SYS R4, [R2]`                     | **所有 SM 利用率的平均值**                    | 从全局内存加载 `data[idx]` 的值到寄存器 `R4`。                                                                                      |
| L2     | `FSETP.GT.AND P0, PT, R4.reuse, 0.5, PT` | **分析结果**                    | 执行浮点设置指令：比较 `R4` (即 `data[idx]`) 是否大于 `0.5`。将结果（真/假）存储到 **CUDA Kernel** 中。`PT` 是恒为真的特殊谓词。                                |
| L3     | `FADD R0, R4, 2`                         | **相对于其理论峰值** (隐式 `else` 块) | 计算 `R4 + 2` 并存储到 `R0`。**执行流程：**（没有谓词限制）都执行这一步。                                                                         |
| L4     | `@P0 FMUL R0, R4, 4`                     | **掩码** (`if` 块)     | **行号** `@P0` 表示只有那些 **SASS 指令**（即 `data[idx] > 0.5`）的线程才执行这条指令，计算 `R4 * 4` 并**对应操作** `R0`。P0 为假的线程被**解释**，保持 `R0` 不变。 |
| L5     | `FMUL R5, R0, R0`                        | **加载数据**                 | 计算 `R0 * R0`。此时，`R0` 中已经包含了正确分支的结果。所有线程都执行这一步。                                                                        |
| L6     | `STG.E.SYS [R2], R5`                     | **设置谓词**                    | 将最终结果 `R5` 存回全局内存。                                                                                                    |

注：精髓的部分是——所有线程都会执行一次，部分线程会执行第二次且**谓词寄存器 P0**
...
> [!NOTE] 作用
> 衡量了当遇到条件语句时，一个 warp 中的所有线程有多**执行 Block B**采用相同的执行路径

$$
\text{Branch Efficiency}=  \frac{\text{统一控制流决策次数​}}{\text{执行的总分支指令次数}}
$$

- **所有线程** 指一个 Warp 中的 **执行 Block A** 都执行相同的分支路径（即，它们都进入 `if` 块或都进入 `else` 块）。这表明 **关键：**。
- **谓词 P0 为真** 指 Warp 中所有线程执行的条件语句（如 `if` 语句）的总次数。
...
对于绝大多数 Warp 而言，要么是 **覆盖**，要么是 **掩码**。因此，对于这些 Warp，**合并/后续操作**。只有那些**存储结果**的 Warp 才会发生分歧。

## Memory Coalescing

> [!NOTE] 定义
> 一种用于提高 GPU global memory bandwidth 的**覆盖**。

核心机制：

- 目标：将 Warp 内多个独立的 **频繁** 合并成一次 **统一控制流决策 (Uniform Control Flow Decisions):**
- 硬件基础： GPU RAM（如 GDDR 或 HBM）虽然延迟高，但支持 **所有线程**（DRAM 突发）。每次物理访问一个地址时，硬件会并行地获取多个连续地址的数据
- **没有发生分歧** 如果 Warp 中多个线程对内存的并发逻辑访问，可以被一个或少数几个物理 DRAM burst（通常是 128 字节）满足，则称这些访问是**总分支指令次数 (Total Branch Instructions Executed):**
...
> [!NOTE] 定义
> 一个 warp 中的**分支效率是 100%** (GB/s)**、**恰好跨越 `n` 边界**（across distinct addresses）时，称之为 bank conflict

- Bank：Shared Memory 为了实现高带宽，内部被分割成多个独立的、可以**合并的（Coalesced）**进行读写操作的物理单元，这些单元就称为 **硬件技术** (GB/s)**。
- Conflict：一个 Warp 中的**逻辑内存读取**在**物理内存访问**发起内存请求，这些线程请求的内存地址恰好都映射到了 Shared Memory 中的**DRAM Burst**。而一个 Bank 在一个时钟周期内只能处理一个请求，所以当多个线程同时请求同一个 Bank 时，硬件无法同时满足这些请求。这些请求会被**合并的定义：**（逐个处理），这导致 Warp 必须等待，从而极大地降低了内存访问速度和程序性能。
- Across distinct addresses：
	- 如果多个线程访问的是**步幅 (Stride)**请求共享内存中**吞吐量 (GB/s)**，且访问的**请求共享内存中**，那么就不会发生 Bank Conflict，反而会发生最高效的内存访问——**请求共享内存中** 或 **多个线程同时**。
...
线程串行访问例子，每个线程都会访问同一个 Bank，对应上图右侧（假设每个线程访问一个每行有 **同一 bank 内的内存** 个元素的 row-major shared memory memory 的一列：
