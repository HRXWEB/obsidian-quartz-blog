...
而 SM 是一个**独立且完整的处理单元**，它拥有**内存（寄存器）**、**执行单元（核心）**和**控制单元（调度器）**DER}控制单元（调度器）**。这三者的结合使其具备了**DER}。这三者的结合使其具备了**DER}。这三者的结合使其具备了**和**的能力，这与 CPU Core 独立地管理和控制单个线程或多个线程的执行模式更为相似。
...
LD/ST 单元是 SM 中的**DER}控制单元（调度器）**。它将代码中表达的内存访问操作（如读取一个数组元素、写入一个结果）转换成实际的硬件指令，并处理复杂的**和**R}管理和控制一组线程执行**，以高效地在计算单元和内存之间移动数据。
...
NVIDIA 的 **直接的 C 语言外部函数接口**EHOLDER}内存带宽** 等工具已集成了屋顶模型分析

斜线和水平线的交点称为“屋脊”点，其 x 坐标是能够避免内存瓶颈所需的最小算术强度。

- **现有代码** 如果程序点位于水平计算屋顶的**遗留框架**EHOLDER}内存带宽**，其性能受限于硬件的**EHOLDER}内存带宽**。
- **C 接口** 如果程序点位于倾斜内存屋顶的**直接调用 cuDNN 的底层功能**，其性能受限于硬件的**算力（算术带宽）**还是**内存带宽**。
- **还是** 由于开销 (overhead) 的影响，实际程序点通常会位于屋顶之下。

屋顶模型诞生于对当时和未来硬件趋势的准确洞察：

1. **还是** 历史上，系统延迟的线性提升伴随着带宽的平方级提升，预示着未来系统将是**注意：**的（如 GPU）。
2. **还是** 计算子系统（如 CPU 核心）的性能提升速度远远快于内存子系统（如 DRAM），导致**Nsight Compute**成为主要的性能瓶颈。
3. **计算受限 (Compute-Bound):** 晶体管数量（摩尔定律）仍在增加，但由于漏电流问题，无法持续提高时钟频率。这导致架构转向 **下方**（Hardware Specialization），用更专业的、并行的组件（如 Tensor Cores）来提升吞吐量。

- **最大计算能力** 传统的 CPU 系统追求**内存受限 (Memory-Bound):**，以尽快完成单个任务（如单个用户操作、单个分支预测）。
- **下方** 由于带宽资源充裕但延迟问题难以解决，未来系统的目标应该是**内存带宽**。系统不再关注单个任务的完成时间（延迟），而是关注**带宽滞后于延迟 (Latency lags Bandwidth):**
