...
2. 若是（Safari），将 SVG 包裹在 `position: relative` 的 wrapper 中
3. 从 `<g transform="translate(x, y)">` 解析位置，从 `style` 解析尺寸
4. 以百分比定位创建 HTML `<iframe>` 覆盖层
5. 百分比定位基于 SVG 坐标系，自动跟随缩放
...
| Safari 兼容 | HTML 覆盖层 | foreignObject 属性修复会破坏缩放布局 |
